...
decl_init_lpfn_table_begin(g_lpfns,
                           g_bncfns,
                           g_blpfnInit); // set g_blpfnInit to true

create_suite_entries(g_lpfns, g_bncfns, G_RANK);
...
char *g_lpszDefaultInput = "2048_16.txt"; // Change to whatever you want 
...
LPFNKMEANS
choose_kmeans_impl(char *lpszImpl, int nRank, int nCenters)
{
    // "raw"
    // "constmem"
    // "constmemset"
...
LPFNBNC
choose_kmeans_bnc(char *lpszImpl, int nRank, int nCenters)
{
    // "raw"
    // "constmem"
    // "constmemset"
...
addBenchmarkSpecOptions(OptionParser &op)
{
    ...
    op.addOption(
        "centers", OPT_INT, "64", "An integer-valued centers argument");
    ...
    // op.addOption("cpu", OPT_BOOL, "0", "perform accumulation on CPU
    // instead");
}
...
try
{
    ...
    {
        fprintf(stderr,
                "failed to select valid implementation for %s(RANK=%d, "
                "CENTERS=%d)!\n",
                g_vKMeansVersion,
                g_nRank,
                g_nCenters);
