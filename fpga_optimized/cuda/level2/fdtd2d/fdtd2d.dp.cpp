...
void init_arrays(size_t NX, size_t NY, size_t tmax, DATA_TYPE *_fict_,
...
void runFdtd(size_t NX, size_t NY, size_t tmax, DATA_TYPE *_fict_,
             DATA_TYPE *ex, DATA_TYPE *ey, DATA_TYPE *hz) {
...
void fdtd_step1_kernel(size_t NX, size_t NY, const DATA_TYPE _fict_,
                       DATA_TYPE *ey, const DATA_TYPE *hz,
                       sycl::nd_item<2> item_ct1) {
  int j = item_ct1.get_global_id(1);
...
void fdtd_step2_kernel(size_t NX, size_t NY, DATA_TYPE *ex, const DATA_TYPE *hz,
                       sycl::nd_item<2> item_ct1) {
  int j = item_ct1.get_global_id(1);
  int i = item_ct1.get_global_id(0);
...
void fdtd_step3_kernel(size_t NX, size_t NY, const DATA_TYPE *ex,
                       const DATA_TYPE *ey, DATA_TYPE *hz,
                       sycl::nd_item<2> item_ct1) {
  int j = item_ct1.get_global_id(1);
  int i = item_ct1.get_global_id(0);
...
void fdtdCuda(size_t NX, size_t NY, size_t tmax, DATA_TYPE *_fict_,
              DATA_TYPE *ex, DATA_TYPE *ey, DATA_TYPE *hz,
              DATA_TYPE *hz_outputFromGpu, ResultDatabase &DB, OptionParser &op,
              size_t device_idx) {
  ...
  sycl::range<2> block(DIM_THREAD_BLOCK_Y, DIM_THREAD_BLOCK_X);
  sycl::range<2> grid((size_t)ceil(((float)NX) / ((float)block[0])),
                      (size_t)ceil(((float)NY) / ((float)block[1])));

  // Transfer the inout data to the device.
  //
  queue.memcpy(_fict_gpu, _fict_, sizeof(DATA_TYPE) * tmax);
  queue.memcpy(ex_gpu, ex, sizeof(DATA_TYPE) * NX * (NY + 1));
  queue.memcpy(ey_gpu, ey, sizeof(DATA_TYPE) * (NX + 1) * NY);
  queue.memcpy(hz_gpu, hz, sizeof(DATA_TYPE) * NX * NY);
  queue.wait_and_throw();

  // Start the calculation.
  //
  sycl::event iteration_begin = queue.ext_oneapi_submit_barrier();
  double t_start = rtclock();

  FPGA_PWR_MEAS_START
  for (int t = 0; t < tmax; t++) {
    const DATA_TYPE fict_val = _fict_[t];
    sycl::event e1_event = queue.submit([&](sycl::handler &cgh) {
      cgh.depends_on({iteration_begin});
      cgh.parallel_for<class fdtd_step1>(
          sycl::nd_range<2>(grid * block, block),
          [=](sycl::nd_item<2> item_ct1)
              [[intel::kernel_args_restrict, intel::num_simd_work_items(16),
                intel::no_global_work_offset(1),
                sycl::reqd_work_group_size(1, DIM_THREAD_BLOCK_Y,
                                           DIM_THREAD_BLOCK_X),
                intel::max_work_group_size(1, DIM_THREAD_BLOCK_Y,
                                           DIM_THREAD_BLOCK_X)]] {
                fdtd_step1_kernel(NX, NY, fict_val, ey_gpu, hz_gpu, item_ct1);
              });
    });
    sycl::event e2_event = queue.submit([&](sycl::handler &cgh) {
      cgh.depends_on({iteration_begin});
      cgh.parallel_for<class fdtd_step2>(
          sycl::nd_range<2>(grid * block, block),
          [=](sycl::nd_item<2> item_ct1)
              [[intel::kernel_args_restrict, intel::num_simd_work_items(16),
                intel::no_global_work_offset(1),
    ...
    iteration_begin = queue.submit([&](sycl::handler &cgh) {
      cgh.depends_on({e1_event, e2_event});
      cgh.parallel_for<class fdtd_step3>(
          sycl::nd_range<2>(grid * block, block),
          [=](sycl::nd_item<2> item_ct1)
              [[intel::kernel_args_restrict, intel::num_simd_work_items(16),
                intel::no_global_work_offset(1),
...
void RunBenchmark(ResultDatabase &DB, OptionParser &op, size_t device_idx) {
  ...
  // Warmup
  fdtdCuda(NX, NY, tmax, _fict_, ex, ey, hz, hz_outputFromGpu, DB, op, device_idx);
