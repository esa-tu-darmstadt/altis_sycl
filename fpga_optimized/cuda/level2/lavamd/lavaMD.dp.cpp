...
//
// origin: 	14 APR 2011 Lukasz G. Szafaryn, from Rodinia
...
void addBenchmarkSpecOptions(OptionParser &op) {
  op.addOption("boxes1d", OPT_INT, "0",
               "specify number of boxes in single dimension, total box "
               "number is that^3");
}
...
void RunBenchmark(ResultDatabase &resultDB, OptionParser &op,
  ...
  for (int i = 0; i < passes; i++) {
    ...
    runTest(resultDB, op, boxes1d, device_idx);
    if (!quiet)
      printf("Done.\n");
  }
}
...
void runTest(ResultDatabase &resultDB, OptionParser &op, int boxes1d,
  ...
  for (i = 0; i < dim_cpu.space_elem; i = i + 1) {
    rv_cpu[i].v = (i % 10 + 1) / 10.0; // get a number in the range 0.1 - 1.0
    rv_cpu[i].x = (i % 10 + 1) / 10.0; // get a number in the range 0.1 - 1.0
    rv_cpu[i].y = (i % 10 + 1) / 10.0; // get a number in the range 0.1 - 1.0
    rv_cpu[i].z = (i % 10 + 1) / 10.0; // get a number in the range 0.1 - 1.0
  }
  ...
  for (i = 0; i < dim_cpu.space_elem; i = i + 1)
    qv_cpu[i] = (i % 10 + 1) / 10.0; // get a number in the range 0.1 - 1.0
