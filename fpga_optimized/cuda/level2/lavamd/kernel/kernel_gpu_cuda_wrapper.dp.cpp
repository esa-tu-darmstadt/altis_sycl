...
void kernel_gpu_cuda_wrapper(par_str par_cpu, dim_str dim_cpu, box_str *box_cpu,
                             ...
                             FOUR_VECTOR *fv_cpu, ResultDatabase &resultDB,
                             OptionParser &op, sycl::queue &queue) {
  ...
  sycl::range<1> threads(NUMBER_THREADS);
  sycl::range<1> blocks(dim_cpu.number_boxes);

  box_str *d_box_gpu = (box_str *)sycl::malloc_device(dim_cpu.box_mem, queue);
  FOUR_VECTOR *d_rv_gpu =
      (FOUR_VECTOR *)sycl::malloc_device(dim_cpu.space_mem, queue);
  ...
  sycl::event k_event = queue.submit([&](sycl::handler &cgh) {
    cgh.parallel_for<class lavamd_kernel>(
        ...
        [=](sycl::nd_item<1> item_ct1)
            [[intel::kernel_args_restrict, intel::num_simd_work_items(1),
              intel::no_global_work_offset(1),
              sycl::reqd_work_group_size(1, 1, NUMBER_THREADS),
              intel::max_work_group_size(1, 1, NUMBER_THREADS)]] {
              kernel_gpu_cuda(par_cpu, dim_cpu, d_box_gpu, d_rv_gpu, d_qv_gpu,
                              d_fv_gpu, item_ct1);
            });
  });
  k_event.wait();
