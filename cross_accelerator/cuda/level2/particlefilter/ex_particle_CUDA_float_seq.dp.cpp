...
constexpr int threads_per_block = 256; // 256 on P630, else 512
 ...
 *****************************/
void
find_index_kernel(double          *arrayX,
                  double          *arrayY,
                  double          *CDF,
                  double          *u,
                  double          *xj,
...
void
normalize_weights_kernel(double          *weights,
                         int              Nparticles,
                         double          *partial_sums,
                         sycl::nd_item<3> item_ct1,
                         double          *sumWeights)
...
void
normalize_weights_kernel2(double          *weights,
                         int              Nparticles,
                         double          *CDF,
                         double          *u,
                         int             *seed,
...
void
normalize_weights_kernel3(double          *weights,
                         int              Nparticles,
                         double          *u,
                         sycl::nd_item<3> item_ct1,
                         double          *u1)
...
void
sum_kernel(double *partial_sums, int Nparticles, sycl::nd_item<3> item_ct1)
{
    int block_id = item_ct1.get_group(2);
    int i = item_ct1.get_local_range(2) * block_id + item_ct1.get_local_id(2);
                  ...
                  double          *buffer)
{
    ...
    if (i < Nparticles)
    {
        ...
        weights[i]
            = 1 / ((double)(Nparticles)); // Donnie - moved this line from end
                                          // of find_index_kernel to prevent all
                                          // weights from being reset before
    ...
    for (int k = 1; k < Nfr; k++)
    {
        ...
        queue.submit([&](sycl::handler &cgh) {
            sycl::accessor<double,
                ...
                buffer_acc_ct1(sycl::range<1>(512), cgh);

            cgh.parallel_for<class likelihood_kernel_id>(
                sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks)
                                      * sycl::range<3>(1, 1, threads_per_block),
                                  sycl::range<3>(1, 1, threads_per_block)),
                [=](sycl::nd_item<3> item_ct1) ATTRIBUTE {
                    likelihood_kernel(arrayX_GPU,
                                      arrayY_GPU,
                                      xj_GPU,
                                      yj_GPU,
                                      ind_GPU,
        ...
        queue.parallel_for<class sum_kernel_id>(
            sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks)
                                  * sycl::range<3>(1, 1, threads_per_block),
                              sycl::range<3>(1, 1, threads_per_block)),
            [=](sycl::nd_item<3> item_ct1)
                ATTRIBUTE { sum_kernel(partial_sums, Nparticles, item_ct1); }).wait();
        queue.submit([&](sycl::handler &cgh) {
            sycl::accessor<double,
                           0,
                           sycl::access_mode::read_write,
            ...
            cgh.parallel_for<class normalize_weights_kernel_id>(
                sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks)
                                      * sycl::range<3>(1, 1, threads_per_block),
                                  sycl::range<3>(1, 1, threads_per_block)),
                [=](sycl::nd_item<3> item_ct1) ATTRIBUTE {
                    normalize_weights_kernel(weights_GPU,
                                             Nparticles,
                                             partial_sums,
                                             item_ct1,
                                             sumWeights_acc_ct1.get_pointer());
                });
        }).wait(); 
        queue.submit([&](sycl::handler &cgh) {
            cgh.parallel_for<class normalize_weights_kernel2_id>(
                sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks)
                                      * sycl::range<3>(1, 1, threads_per_block),
                                  sycl::range<3>(1, 1, threads_per_block)),
                [=](sycl::nd_item<3> item_ct1) ATTRIBUTE {
                    normalize_weights_kernel2(weights_GPU,
                                             Nparticles,
                                             CDF_GPU,
                                             u_GPU,
                                             seed_GPU,
        ...
        queue.submit([&](sycl::handler &cgh) {
            sycl::accessor<double,
                           ...
                           sycl::access::target::local>
                u1_acc_ct1(cgh);
            cgh.parallel_for<class normalize_weights_kernel3_id>(
                sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks)
                                      * sycl::range<3>(1, 1, threads_per_block),
                                  sycl::range<3>(1, 1, threads_per_block)),
                [=](sycl::nd_item<3> item_ct1) ATTRIBUTE {
                    normalize_weights_kernel3(weights_GPU,
                                             Nparticles,
                                             u_GPU,
                                             item_ct1,
                                             u1_acc_ct1.get_pointer());
                });
        }).wait(); 
        queue.parallel_for<class find_index_kernel_id>(
            sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks)
                                  * sycl::range<3>(1, 1, threads_per_block),
                              sycl::range<3>(1, 1, threads_per_block)),
            [=](sycl::nd_item<3> item_ct1) ATTRIBUTE {
                find_index_kernel(arrayX_GPU,
                                  arrayY_GPU,
                                  CDF_GPU,
                                  u_GPU,
                                  xj_GPU,
