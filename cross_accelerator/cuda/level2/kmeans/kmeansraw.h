...
#ifdef _FPGA
#define ATTRIBUTE                                            \
    [[sycl::reqd_work_group_size(1, 1, THREADBLOCK_SIZE), \
      intel::max_work_group_size(1, 1, THREADBLOCK_SIZE)]]
#define ACCESS_SPACE sycl::access::address_space::global_device_space
#else
#define ACCESS_SPACE sycl::access::address_space::global_space
#define ATTRIBUTE
#endif
...
template<int R, int C>
void
resetExplicit(float *pC, int *pCC, sycl::nd_item<3> item_ct1)
{
...
template<int R, int C>
void
finalizeCentersBasic(float *pC, int *pCC, sycl::nd_item<3> item_ct1)
{
    int idx = item_ct1.get_group(2) * item_ct1.get_local_range(2)
              + item_ct1.get_local_id(2);
    if (idx >= C)
...
void
accumulateCenters(
    float *pP, float *pC, int *pCC, int *pCI, int nP, sycl::nd_item<3> item_ct1)
{
    int idx = item_ct1.get_group(2) * item_ct1.get_local_range(2)
              + item_ct1.get_local_id(2);
    if (idx >= nP)
...
public:
    ...
    {
        queue.parallel_for<class accumulator_gm_reset_rwomaj_kernel_id>(
        sycl::nd_range<3>(
    ...
    static void accumulate(
        float *pP, float *pC, int *pCC, int *pCI, int nP, sycl::queue &queue)
    {
        const uint nPointsBlocks = iDivUp(nP, THREADBLOCK_SIZE);
        queue.parallel_for<class accumulator_gm_acc_rowmaj_kernel_id>(
            sycl::nd_range<3>(sycl::range<3>(1, 1, nPointsBlocks)
                                    * sycl::range<3>(1, 1, THREADBLOCK_SIZE),
                                sycl::range<3>(1, 1, THREADBLOCK_SIZE)),
            [=](sycl::nd_item<3> item_ct1) ATTRIBUTE {
                accumulateCenters<R, C>(pP, pC, pCC, pCI, nP, item_ct1);
            }).wait();
    }
    ...
    static void finalize(float *pC, int *pCC, sycl::queue &queue)
    {
        ...
        queue.parallel_for<class accumulator_gm_finalize_rowmaj_kernel_id>(
            sycl::nd_range<3>(sycl::range<3>(1, 1, nAccums)
                                    * sycl::range<3>(1, 1, THREADBLOCK_SIZE),
                                sycl::range<3>(1, 1, THREADBLOCK_SIZE)),
            [=](sycl::nd_item<3> item_ct1) ATTRIBUTE {
                   ...
                   sycl::nd_item<3> item_ct1)
{
    ...
    pCI[idx] = nearestCenter<R, C>(&pP[idx * R], pC);
}
...
public:
    ...
    bool execute(bool warmup)
    {
        if (warmup)
        {
            updatecentersIn(m_dCenters); // deal with centers data
            mapCenters(m_dPoints, m_dClusterIds, m_nPoints);
            resetAccumulators(m_dCenters, m_dClusterCounts);
            accumulate(m_dPoints,
                        m_dCenters,
                        m_dClusterCounts,
                        m_dClusterIds,
                        m_nPoints);
...
protected:
    ...
    void accumulate(float *pP, float *pC, int *pCC, int *pCI, int nP)
    {
        m_accumulator.accumulate(pP, pC, pCC, pCI, nP, queue);
    }
    ...
    void mapCenters(float *pP, int *pCI, int nP)
    {
        ...
        queue.submit([&](sycl::handler &cgh) {
            cgh.parallel_for<map_centers_kernel_rowmaj_id>(
                sycl::nd_range<3>(
...
#pragma warning(default : 4996)
    ...
    static void MyPrintCenters(float *pCenters,
                               int    nColLimit = 8,
                               int    nRowLimit = 16)
    {
        nRowLimit = (nRowLimit == 0) ? C : std::min(nRowLimit, C);
    ...
    static void PrintCentersTransposed(FILE  *fp,
                                       float *pCenters,
                                       int    nColLimit = 8,
                                       int    nRowLimit = 16)
    {
...
public:
                            ...
                            bool            bVerbose)
    {
        ...
        // Warmup for preciser timing-measurements!
        //
        (*pKMeans).execute(true);
        queue.wait_and_throw();
        ...
        if (bVerbose)
        {
            ...
            // shrLogEx(LOGBOTH | MASTER, 0, "kmeans, Throughput = %.4f MB/s,
            // Time = %.5f s, Size = %u Bytes, NumDevsUsed = %u, Workgroup =
            // %u\n", (1.0e-6 * (double)byteCount / dAvgSecs), dAvgSecs,
            //byteCount, 1, THREADBLOCK_SIZE);
        }
                        ...
                        bool            bVerbose)
    {
        ...
        if (bVerify)
        {
            ...
            double dAvgSecs = 0;
            PrintResults(stdout, centers, refcenters, bSuccess, dAvgSecs,
                dAvgSecs, bVerbose);
        }

        std::cout << "Cleaning up..." << std::endl;
        if (h_Centers) free(h_Centers);
    }
};
