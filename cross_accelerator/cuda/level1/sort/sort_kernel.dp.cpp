...
uint
scanLSB(const uint val, uint *s_data, sycl::nd_item<3> item_ct1)
{
...
sycl::uint4
scan4(sycl::uint4 idata, uint *ptr, sycl::nd_item<3> item_ct1)
{
...
SYCL_EXTERNAL void
radixSortBlocks(const uint       nbits,
                const uint       startbit,
                sycl::uint4     *keysOut,
                sycl::uint4     *valuesOut,
                sycl::uint4     *keysIn,
                sycl::uint4     *valuesIn,
...
////////////////////////////////////////////////////////////////////////////////////////////////////
/// <summary>	Given an array with blocks sorted according to a 4-bit radix
/// group, each
...
/// <param name="counters">	   	[in,out] If non-null, the counters.
/// </param> <param name="blockOffsets">	[in,out] If non-null, the block
/// offsets. </param> <param name="startbit">	   	The startbit. </param>
/// <param name="totalBlocks"> 	The total blocks. </param>
////////////////////////////////////////////////////////////////////////////////////////////////////

SYCL_EXTERNAL void
findRadixOffsets(sycl::uint2     *keys,
                 uint            *counters,
                 uint            *blockOffsets,
                 uint             startbit,
                 uint             totalBlocks,
                 ...
                 uint            *sStartPointers)
{
    ...
    dpct_local[2 * localId]     = (radix2.x() >> startbit) & 0xF;
    dpct_local[2 * localId + 1] = (radix2.y() >> startbit) & 0xF;

    // Finds the position where the dpct_local entries differ and stores start
    // index for each radix.
    if (localId < 16)
        sStartPointers[localId] = 0;
    item_ct1.barrier(sycl::access::fence_space::local_space);

    if ((localId > 0) && (dpct_local[localId] != dpct_local[localId - 1]))
        sStartPointers[dpct_local[localId]] = localId;
    if (dpct_local[localId + groupSize] != dpct_local[localId + groupSize - 1])
        sStartPointers[dpct_local[localId + groupSize]] = localId + groupSize;
    item_ct1.barrier(sycl::access::fence_space::local_space);
    ...
    if ((localId > 0) && (dpct_local[localId] != dpct_local[localId - 1]))
        sStartPointers[dpct_local[localId - 1]]
            = localId - sStartPointers[dpct_local[localId - 1]];
    if (dpct_local[localId + groupSize] != dpct_local[localId + groupSize - 1])
        sStartPointers[dpct_local[localId + groupSize - 1]]
            = localId + groupSize
              - sStartPointers[dpct_local[localId + groupSize - 1]];

    if (localId == groupSize - 1)
        sStartPointers[dpct_local[2 * groupSize - 1]]
            = 2 * groupSize - sStartPointers[dpct_local[2 * groupSize - 1]];
    item_ct1.barrier(sycl::access::fence_space::local_space);
...
////////////////////////////////////////////////////////////////////////////////////////////////////
/// <summary>	reorderData shuffles data in the array globally after the radix
/// offsets have been found. On compute version 1.1 and earlier GPUs, this code
/// depends on SORT_BLOCK_SIZE being 16 * number of radices (i.e. 16 *
/// 2^nbits).. </summary>
///
/// <remarks>	Ed, 5/19/2020. </remarks>
///
/// <param name="startbit">	   	The startbit. </param>
/// <param name="outKeys">	   	[in,out] If non-null, the out keys.
/// </param>
/// <param name="outValues">   	[in,out] If non-null, the out values. </param>
/// <param name="keys">		   	[in,out] If non-null, the keys. </param>
/// <param name="values">	   	[in,out] If non-null, the values.
/// </param> <param name="blockOffsets">	[in,out] If non-null, the block
/// offsets. </param>
/// <param name="offsets">	   	[in,out] If non-null, the offsets.
/// </param> <param name="sizes">	   	[in,out] If non-null, the sizes.
/// </param> <param name="totalBlocks"> 	The total blocks. </param>
////////////////////////////////////////////////////////////////////////////////////////////////////

SYCL_EXTERNAL void
reorderData(uint             startbit,
            uint            *outKeys,
            uint            *outValues,
            sycl::uint2     *keys,
            sycl::uint2     *values,
...
uint
scanLocalMem(const uint val, uint *s_data, sycl::nd_item<3> item_ct1)
{
...
/// <remarks>	Ed, 5/20/2020. </remarks>
///
/// <param name="g_odata">	  	[in,out] If non-null, the odata.
/// </param> <param name="g_idata">	  	[in,out] If non-null, the idata.
/// </param> <param name="g_blockSums">	[in,out] If non-null, the block sums.
/// </param> <param name="n">		  	An int to process. </param>
...
SYCL_EXTERNAL void
vectorAddUniform4(uint            *d_vector,
                  const uint      *d_uniforms,
                  const int        n,
                  sycl::nd_item<3> item_ct1,
                  uint            *uni)
{
