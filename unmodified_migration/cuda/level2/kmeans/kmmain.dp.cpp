...
void RunBenchmark(ResultDatabase &DB, OptionParser &op) try {
    int dev = op.getOptionInt("device");
    /*
    DPCT1093:541: The "dev" device may be not the one intended for use. Adjust
    the selected device if needed.
    */
    CUDA_SAFE_CALL(DPCT_CHECK_ERROR(dpct::select_device(dev)));
