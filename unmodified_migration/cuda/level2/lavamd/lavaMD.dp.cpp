...
void runTest(ResultDatabase &resultDB, OptionParser &op, int boxes1d) {
    ...
    if (uvm) {
        /*
        DPCT1064:935: Migrated cudaMallocManaged call is used in a
        macro/template definition and may not be valid for all macro/template
        uses. Adjust the code.
        */
        checkCudaErrors(
            DPCT_CHECK_ERROR(box_cpu = (box_str *)sycl::malloc_shared(
    ...
    if (uvm) {
        /*
        DPCT1064:936: Migrated cudaMallocManaged call is used in a
        macro/template definition and may not be valid for all macro/template
        uses. Adjust the code.
        */
        checkCudaErrors(DPCT_CHECK_ERROR(
            rv_cpu = (FOUR_VECTOR *)sycl::malloc_shared(
    ...
    if (uvm) {
        /*
        DPCT1064:937: Migrated cudaMallocManaged call is used in a
        macro/template definition and may not be valid for all macro/template
        uses. Adjust the code.
        */
        checkCudaErrors(DPCT_CHECK_ERROR(
            qv_cpu = (double *)sycl::malloc_shared(dim_cpu.space_mem2,
    ...
    if (uvm) {
        /*
        DPCT1064:938: Migrated cudaMallocManaged call is used in a
        macro/template definition and may not be valid for all macro/template
        uses. Adjust the code.
        */
        checkCudaErrors(DPCT_CHECK_ERROR(
            fv_cpu = (FOUR_VECTOR *)sycl::malloc_shared(
                dim_cpu.space_mem, dpct::get_default_queue())));
    } else {
    ...
    if (uvm) {
        checkCudaErrors(
            DPCT_CHECK_ERROR(sycl::free(rv_cpu, dpct::get_default_queue())));
        checkCudaErrors(
            DPCT_CHECK_ERROR(sycl::free(qv_cpu, dpct::get_default_queue())));
        checkCudaErrors(
            DPCT_CHECK_ERROR(sycl::free(fv_cpu, dpct::get_default_queue())));
        checkCudaErrors(
            DPCT_CHECK_ERROR(sycl::free(box_cpu, dpct::get_default_queue())));
    } else {
        free(rv_cpu);
        free(qv_cpu);
        free(fv_cpu);
