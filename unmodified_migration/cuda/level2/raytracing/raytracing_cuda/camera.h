...
/*
DPCT1032:489: A different random number generator is used. You may need to
adjust the code.
*/
vec3 random_in_unit_disk(
    ...
    vec3 p;
    do {
        p = 2.0f * vec3(local_rand_state->generate<
                            oneapi::mkl::rng::device::uniform<float>, 1>(),
                        local_rand_state->generate<
                            oneapi::mkl::rng::device::uniform<float>, 1>(),
                        0) -
            vec3(1, 1, 0);
    } while (dot(p,p) >= 1.0f);
...
public:
    ...
    /*
    DPCT1032:490: A different random number generator is used. You may need to
    adjust the code.
    */
    ray
